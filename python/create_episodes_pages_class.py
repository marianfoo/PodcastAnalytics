...
def createEpisodePages(show,permalink_show,folder_name,grand_parent,max_episodes):
    track = 1
    for episode in metadata_json[show]['all_episodes']:
        if track > max_episodes:
            break
        episode_id = episode
...
def createAllPages(max_episodes = 9999, debug = False):
    if debug == True:
        print('delete all pages')
        removeAllPages()
    createEpisodePages('festflauschig','fest-flauschig','festflauschig','Fest und Flauschig',max_episodes)
    createEpisodePages('baywatchberlin','baywatch-berlin','baywatchberlin','Baywatch Berlin',max_episodes)
    createEpisodePages('sanftsorgfaeltig','sanft-sorgfältig','sanftsorgfältig','Sanft und Sorgfältig',max_episodes)
    createEpisodePages('gemischtes-hack','gemischtes-hack','gemischteshack','Gemischtes Hack',max_episodes)
    createEpisodePages('evds','eulen-vor-die-saeue','eulenvordiesaeue','Eulen vor die Säue',max_episodes)

def removeAllPages():
    pages = ['festflauschig','baywatchberlin','sanftsorgfältig','gemischteshack','eulenvordiesaeue']
    for page in pages:
        for file in Path(str(Path().resolve().parent.resolve()) + '/PodcastAnalytics/docs/_pages/' + page + '/episodes/' ).glob('**/*.md'):
            file.unlink()